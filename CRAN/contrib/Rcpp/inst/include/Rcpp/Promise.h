...
namespace Rcpp{
    ...
    public:
         ...
         */
        SEXP value() const{
            SEXP val = get_PRVALUE( Storage::get__() ) ;
            if( val == R_UnboundValue ) throw unevaluated_promise() ;
            return val ;
        }

        bool was_evaluated() const {
            return get_PRVALUE(Storage::get__()) != R_UnboundValue ;
        }
        ...
        ExpressionVector expression() const {
            return ExpressionVector(get_PRCODE( Storage::get__() )) ;
        }
        ...
        Environment environment() const {
            return Environment( get_PRENV(Storage::get__() ) ) ;
        }
